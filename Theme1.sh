...
exit 0
#TG通知
if [ -n "$FOLDERS" ]; then  curl "https://api.telegram.org/bot${{ secrets.TELEGRAM_BOT_TOKEN }}/sendMessage" -d "chat_id="${{ secrets.TELEGRAM_CHAT_ID }}"&text=🚫插件库同步失败，分支：${{env.TARGET_PROFILE}}${{matrix.target}}，失败列表：$FOLDERSX......"; else curl "https://api.telegram.org/bot${{ secrets.TELEGRAM_BOT_TOKEN }}/sendMessage" -d "chat_id="${{ secrets.TELEGRAM_CHAT_ID }}"&text=🎉插件库同步成功，分支：Theme1${{matrix.target}}......"; fi
